...
public class SatisfactionSurveyUtils {

	// 配置目录类型
	private static final String FILE_NAME = "config/scale/satisfaction_survey.json";

	// 获取单实例对
	private static SatisfactionSurveyUtils instance = new SatisfactionSurveyUtils();
	 ...
	 * 私有构
	 */
	private SatisfactionSurveyUtils(){
		this.initConfigs();
	}
	...
	private void initConfigs(){
		...
		try{
			...
			if (!file.exists()) {
				url = SatisfactionSurveyUtils.class.getClassLoader().getResource(
						FILE_NAME);
				path = url.getPath();
				file = new File(path);
			}
	 ...
	 * @return 单实例对象
	 */
	public static SatisfactionSurveyUtils getInstance(){
		return instance;
	}
