...
public class Eat26Utils {

	// 配置目录类型
	private static final String FILE_NAME = "config/scale/eat26.json";

	// 获取单实例对
	private static Eat26Utils instance = new Eat26Utils();
	...
	private void initConfigs(){
		...
		try{
			...
			if (!file.exists()) {
				url = Eat26Utils.class.getClassLoader().getResource(
						FILE_NAME);
				path = url.getPath();
				file = new File(path);
			}
	 ...
	 * @return 单实例对象
	 */
	public static Eat26Utils getInstance(){
		return instance;
	}
