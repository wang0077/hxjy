...
public class Gad7Utils {

	// 配置目录类型
	private static final String FILE_NAME = "config/scale/gad7.json";

	// 获取单实例对
	private static Gad7Utils instance = new Gad7Utils();
	...
	private void initConfigs(){
		...
		try{
			...
			if (!file.exists()) {
				url = Gad7Utils.class.getClassLoader().getResource(
						FILE_NAME);
				path = url.getPath();
				file = new File(path);
			}
	 ...
	 * @return 单实例对象
	 */
	public static Gad7Utils getInstance(){
		return instance;
	}
