...
public class Phq9Utils {

	// 配置目录类型
	private static final String FILE_NAME = "config/scale/phq9.json";

	// 获取单实例对
	private static Phq9Utils instance = new Phq9Utils();
	...
	private void initConfigs(){
		...
		try{
			...
			if (!file.exists()) {
				url = Phq9Utils.class.getClassLoader().getResource(
						FILE_NAME);
				path = url.getPath();
				file = new File(path);
			}
	 ...
	 * @return 单实例对象
	 */
	public static Phq9Utils getInstance(){
		return instance;
	}
