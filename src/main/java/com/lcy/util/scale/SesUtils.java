...
public class SesUtils {
	...
	// 配置目录类型
	private static final String FILE_NAME = "config/scale/ses.json";

	// 获取单实例对
	private static SesUtils instance = new SesUtils();
	...
	private void initConfigs(){
		...
		try{
			...
			if (!file.exists()) {
				url = SesUtils.class.getClassLoader().getResource(
						FILE_NAME);
				path = url.getPath();
				file = new File(path);
			}
	 ...
	 * @return 单实例对象
	 */
	public static SesUtils getInstance(){
		return instance;
	}
